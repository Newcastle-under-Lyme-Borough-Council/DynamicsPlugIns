...
function CustomStepper(props: any) {
  ...
  // const [toastIdSubmit, setToastIdSubmit]: any = useState(null);
  // const [allServiceRequest, setAllServiceRequest]: any = useState([]);
  const [show, setShow]: any = useState(false);
  const customId = "1";
  // const toastIdNew = useRef<React.ReactText | undefined>(undefined);

  // const notify = () => {
  //   if (
  //     toastIdNew.current === undefined ||
  //     !toast.isActive(toastIdNew.current)
  //   ) {
  //     toastIdNew.current = toast("I cannot be duplicated!");
  //   }
  // };
    ...
    props.EContext.webAPI
      ...
      .then(
        function success(results: any) {
          ...
          if (activeStep === 5 && serviceDetails.ss_description === "") {
            ...
            submitErrorText = `A missed collection for ${serviceDetails.ss_service_name} has already been reported. Reference Number is ${results?.entities[0]?.ss_referencenumber}`;
            showSubmitError(submitErrorText);
          } else {
            var record: any = {};
            record[
            ...
            props.EContext.webAPI.createRecord(entityName, record).then(
              function success(result: any) {
                ...
                // let successText =
                //   serviceDetails.ss_notificationid !== ""
                //     ? "An enquiry has been successfully created."
  ...
  const showSubmitError = (submitErrorText: string) => {
    if (submitErrorText !== "") {
      ...
      // if (toastIdSubmit !== null) {
      //   toast.update(toastIdSubmit, {
      //     render: submitErrorText,
      //     type: toast.TYPE.ERROR,
      //     autoClose: 3000,
      ...
      //     toastId: id,
      //   });
      //   setToastIdSubmit(id); // Cast the ID to string
      // }
    }
  };

  //const breadCrumbData = ['Home', 'Products', 'Category', 'Item']
