...
namespace SS.MSDYN.LGIntelliware.Plugins
{
    public class PostContact : PluginBase
    {
        ...
        protected void Execute(LocalPluginContext localContext)
        {
            ...
            {
                tracingService.Trace("Fault exception occured executing PostContact: {0}.", ex.ToString());
                throw new InvalidPluginExecutionException("Fault exception occured executing PostContact: " + ex.Message + ".");
            }
            catch (Exception ex)
            {
                tracingService.Trace("An exception occured executing PostContact: {0}.", ex.ToString());
